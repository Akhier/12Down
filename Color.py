import libtcodpy


lighter_red = libtcodpy.lighter_red
light_red = libtcodpy.light_red
darker_red = libtcodpy.darker_red
light_green = libtcodpy.light_green
green = libtcodpy.green
darker_green = libtcodpy.darker_green
gray = libtcodpy.gray
dark_gray = libtcodpy.dark_gray
yellow = libtcodpy.yellow
orange = libtcodpy.orange
white = libtcodpy.white
darker_yellow = libtcodpy.darker_yellow
dark_yellow = libtcodpy.dark_yellow
red = libtcodpy.red
sky = libtcodpy.sky
gold = libtcodpy.gold
silver = libtcodpy.silver
dark_green = libtcodpy.dark_green
lighter_green = libtcodpy.lighter_green
darker_orange = libtcodpy.darker_orange
dark_crimson = libtcodpy.dark_crimson
light_blue = libtcodpy.light_blue
blue = libtcodpy.blue
darker_blue = libtcodpy.darker_blue
pink = libtcodpy.pink
black = libtcodpy.black
light_purple = libtcodpy.light_purple
map_tile_visible = libtcodpy.lighter_gray
map_tile_seen = libtcodpy.dark_gray
